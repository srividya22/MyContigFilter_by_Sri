...
class MyContigFilter_by_SriTest(unittest.TestCase):

    @classmethod
    def setUpClass(cls):
        token = environ.get('KB_AUTH_TOKEN', None)
        cls.ctx = {'token': token, 'provenance': [{'service': 'MyContigFilter_by_Sri',
        ...
        for nameval in config.items('MyContigFilter_by_Sri'):
            cls.cfg[nameval[0]] = nameval[1]
        cls.wsURL = cls.cfg['workspace-url']
        cls.wsClient = workspaceService(cls.wsURL, token=token)
        cls.serviceImpl = MyContigFilter_by_Sri(cls.cfg)
    ...
    def getWsName(self):
        ...
        wsName = "test_MyContigFilter_by_Sri_" + str(suffix)
